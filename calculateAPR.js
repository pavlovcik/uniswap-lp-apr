(function calculateAPR() {
    ...
    const CAPTURED = document.body.innerText.match(/^\$[0-9]{1,9}([,.][0-9]{1,9})/igm);
    ...
    console.log(state);
    updateDomNode(`${state.projectedAPR} · APR\n$${(((state.projectedAPR) / 365) * state.liquidity).toFixed(2)} · Daily\n$${(((state.projectedAPR)) * state.liquidity).toFixed(2)} · Annual`);
...
`;
    ...
    function getPositionIdFromUrl() {
        const lastNumbersInUrl = window.location.href.match(/\d+$/igm);
        ...
        const observer = new MutationObserver(function (mutations) {
            mutations.forEach(function (mutation) {
                if (mutation.type === "childList") {
                    ...
                    const newCaptured = document.body.innerText.match(/^\$[0-9]{1,9}([,.][0-9]{1,9})/igm);
    ...
    function updateDomNode(string) {
        state.domNode.innerText = string;
    }

})();

// Jan-21-2022 05:12:52 AM